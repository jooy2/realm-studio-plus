...
// `@sentry/electron` v7 requires the explicit `/main` and `/renderer` subpath
// imports — the bare entrypoint throws at runtime. Types are pulled in via
// `import type` (erased at compile time); the real module is required only on
// the branch that matches the current process.
import type * as MainSentryNS from '@sentry/electron/main';
import type * as RendererSentryNS from '@sentry/electron/renderer';
...
if (process.type === 'renderer') {
  ...
  const Sentry: typeof RendererSentryNS = require('@sentry/electron/renderer');
  Sentry.init({
    dsn,
    environment,
  ...
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const Sentry: typeof MainSentryNS = require('@sentry/electron/main');
