...
- Removed the GitHub Actions workflows inherited from the original project. `publish-release.yml` built through MongoDB's signing infrastructure and uploaded to their S3 bucket; `prepare-release.yml`, `Issue-Needs-Attention.yml` and `check-pr-title.yml` (which failed every pull request that was not prefixed with a JIRA ticket) called reusable actions from `realm/ci-actions`; `issue-labeler.yml` parsed issue forms named `bug.yml` and `feature.yml` that are not in this repository, and labelled them for the .NET SDK's Unity and Xamarin app types; `auto-assign.yml` and `no-response.yml` are Realm's issue triage policy. None of them could run here - the first four for want of secrets this fork does not have. The `.github` configuration files they read, the `actions/sign-windows` and `actions/sign-macos` composite actions that only `publish-release.yml` used, and `evergreen.yml`, a stub for MongoDB's internal CI, went with them.
- Replaced `prs-lint-build-package.yml` with [`ci.yml`](.github/workflows/ci.yml), which lints, tests and builds on pull requests into `main` and on pushes to it. The workflow it replaces only triggered on pull requests into `channel/*` branches, which this fork does not use, and ran `lint:es` and `lint:sass`, two scripts that no longer exist.
- Added `npm run lint:check`, which runs eslint and stylelint without `--fix` so that a linter finding fails the check instead of being silently repaired. `npm run lint` still fixes.
- Removed `npm run check:package-lock` and the script behind it. It read `dependencies` from the lockfile, a key that has not existed since lockfile version 2, and crashed on every run; `npm ci` refuses to install at all when `package.json` and the lockfile disagree, which is the same guarantee.
